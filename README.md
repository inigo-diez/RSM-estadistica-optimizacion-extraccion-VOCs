...
#### Figuras — `Y1_diagnosticos.png` … `Y9_diagnosticos.png`

| Y₁ | Y₂ | Y₃ |
|:--:|:--:|:--:|
| ![Diagnósticos Y1](Graficos/Y1_diagnosticos.png) | ![Diagnósticos Y2](Graficos/Y2_diagnosticos.png) | ![Diagnósticos Y3](Graficos/Y3_diagnosticos.png) |

| Y₄ | Y₅ | Y₆ |
|:--:|:--:|:--:|
| ![Diagnósticos Y4](Graficos/Y4_diagnosticos.png) | ![Diagnósticos Y5](Graficos/Y5_diagnosticos.png) | ![Diagnósticos Y6](Graficos/Y6_diagnosticos.png) |

| Y₇ | Y₈ | Y₉ |
|:--:|:--:|:--:|
| ![Diagnósticos Y7](Graficos/Y7_diagnosticos.png) | ![Diagnósticos Y8](Graficos/Y8_diagnosticos.png) | ![Diagnósticos Y9](Graficos/Y9_diagnosticos.png) |

Panel 2×2 de diagnósticos de residuos por respuesta, generado tras el ajuste OLS cuadrático:
- **Q-Q plot**: contrasta la normalidad de los residuos mediante el test de Shapiro-Wilk; los puntos deben alinearse sobre la recta de referencia.
- **Residuos vs. predichos**: detecta heterocedasticidad o estructura residual no capturada; la banda ideal es aleatoria y centrada en cero.
- **Distancia de Cook**: identifica experimentos influyentes (umbral 4/n); valores superiores al umbral merecen revisión.
- **Residuos estandarizados**: señala outliers (|z| > 2) que pueden distorsionar el ajuste.
...
> **Nota sobre la escala:** todos los valores de respuesta representados corresponden a la **escala real** (back-transformados). Las respuestas con transformación SQRT (Y₁, Y₂, Y₃, Y₅, Y₈, Y₉) se reconvierten como Ŷ² y las LOG (Y₄, Y₆, Y₇) como exp(Ŷ). Las variables de fracción Y₅–Y₉ se multiplican por 100 para expresarse en porcentaje (%). El número de decimales en las isolíneas y etiquetas de máximo se ajusta automáticamente según la magnitud de cada respuesta (0 decimales si el máximo ≥ 10 000, hasta 5 si el máximo < 1), garantizando legibilidad en cualquier escala.

#### Figura — `PredVsObs_Todas.png`

![Predicho vs Observado — 9 respuestas](Graficos/PredVsObs_Todas.png)

Panel 3×3 con gráficos de dispersión Predicho vs. Observado para las 9 respuestas **en escala real** (back-transformados). Se incluyen la línea de identidad y la banda de ±RMSE. Los puntos próximos a la diagonal confirman que el modelo cuadrático ajusta bien los datos. Puntos alejados de la diagonal son experimentos influyentes o outliers que merecen revisión.
...
Para cada respuesta: panel con el punto crítico analítico (solución del sistema ∂Ŷ/∂xᵢ = 0, i = 1, 2, 3), los eigenvalores de la **matriz Hessiana** del modelo cuadrático y la clasificación del punto crítico. El valor predicho Y* se muestra en **escala real** (back-transformado: SQRT→Ŷ², LOG→exp(Ŷ); ×100 para variables de fracción).
...
RSM TFG/
│
├── RSM_Optimization_VOCs.ipynb   ← Notebook principal (análisis completo)
...
└── Graficos/
    ...
    ├── Y[1-9]_diagnosticos.png   ← Diagnósticos de residuos (Q-Q, Cook, etc.)
